...
from can.logger import _create_bus, _parse_filters, _append_filter_argument
...
class CanViewer:
    ...
    def draw_can_bus_message(self, msg, sorting=False):
        ...
        if not sorting:
            ...
            if new_id_added or length_changed:
                ...
                self.ids[key] = {"row": row, "count": 0, "msg": msg, "dt": 0, "dt_min": 0xffffffff, "dt_max":0}
            else:
                # Calculate the time since the last message and save the timestamp
                self.ids[key]["dt"] = msg.timestamp - self.ids[key]["msg"].timestamp

                self.ids[key]["dt_min"] = self.ids[key]["dt"] if self.ids[key]["dt"]<self.ids[key]["dt_min"] else self.ids[key]["dt_min"] 
                self.ids[key]["dt_max"] = self.ids[key]["dt"] if self.ids[key]["dt"]>self.ids[key]["dt_max"]  else self.ids[key]["dt_max"] 
        ...
        self.draw_line(
                self.ids[key]["row"], 23, "{0:.6f} {1:.6f} {2:.6f}".format( self.ids[key]["dt"], self.ids[key]["dt_min"] ,self.ids[key]["dt_max"] ), color
        )
        self.draw_line(self.ids[key]["row"], 35+24, arbitration_id_string, color)
        self.draw_line(self.ids[key]["row"], 47+24, str(msg.dlc), color)
        ...
        if self.data_structs:
            try:
                ...
                self.draw_line(self.ids[key]["row"], 77+24, values_string, color)
    ...
    def draw_header(self):
        ...
        self.draw_line(0, 35+24, "ID", curses.A_BOLD)
        self.draw_line(0, 47+24, "DLC", curses.A_BOLD)
        self.draw_line(0, 52+24, "Data", curses.A_BOLD)
